...
class HelpdeskTeam(models.Model):
    ...
    #    domain="[('name', 'in', [tung, Mitchell Admin])]"
    domain=lambda self: [('groups_id', 'in', self.env.ref('helpdesk_mgmt.group_helpdesk_team_manager').ids)]
    )
    ...
    #     string="Team Leader",
    #     check_company=True,
    #     domain=[('groups_id', 'in', self.env.ref('res_group.group_helpdesk_team_manager').id)]
    ...
    pipeline_id =fields.Many2one(comodel_name="helpdesk.ticket.pipeline", string="Pipeline")
